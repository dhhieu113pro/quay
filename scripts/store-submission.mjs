...
export function selectStorePackages(fileNames, version) {
  const expected = [
    `Quay_${version}_x64.msix`,
    `Quay_${version}_arm64.msix`,
  ];
...
export function buildSubmissionUpdate(createdSubmission, packageNames) {
  ...
  for (const field of EDITABLE_SUBMISSION_FIELDS) {
    if (Object.prototype.hasOwnProperty.call(createdSubmission, field)) {
      update[field] = createdSubmission[field];
    }
  }
...
export async function runStoreSubmission({
  ...
  if (!tokenBody.access_token) {
    throw new Error('Microsoft Entra token response did not contain access_token');
  }
  ...
  const createResponse = await fetchImpl(apiBase, {
    method: 'POST',
    headers: authHeaders,
  });
  const created = await readJson(createResponse);
  if (!created.id || !created.fileUploadUrl) {
    throw new Error('Store create-submission response is missing id or fileUploadUrl');
  }

  const submissionUrl = `${apiBase}/${encodeURIComponent(created.id)}`;
  const updatePayload = buildSubmissionUpdate(created, packageNames);
  const updateResponse = await fetchImpl(submissionUrl, {
    method: 'PUT',
    headers: authHeaders,
  ...
  const uploadResponse = await fetchImpl(created.fileUploadUrl, {
    method: 'PUT',
    headers: {
  ...
  const commitResponse = await fetchImpl(`${submissionUrl}/commit`, {
    method: 'POST',
  ...
  for (let attempt = 0; attempt < maxPollAttempts; attempt += 1) {
    const statusResponse = await fetchImpl(`${submissionUrl}/status`, {
      method: 'GET',
      headers: { authorization: `Bearer ${tokenBody.access_token}` },
    });
    ...
    if (ACCEPTED_STATUSES.has(status)) {
      return { submissionId: created.id, status };
    }
...
async function main() {
  ...
  if (missing.length) {
    throw new Error(`Missing required Store environment values: ${missing.join(', ')}`);
  }
