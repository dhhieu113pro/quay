...
export function EnvEditor({
  ...
  const existing = new Set([
    ...rows.map((row) => row.key.trim()),
    ...inheritedRows.map((row) => row.key.trim()),
  ].filter(Boolean));
  ...
  return (
    <div className="grid gap-2">
      <div className="flex items-center justify-between gap-2">
        <Label>{label}</Label>
        <Button type="button" variant="ghost" size="sm" onClick={() => onChange([...rows, { id: rid(), key: "", value: "" }])}>
      ...
      {available.length ? (
        <div className="flex flex-wrap gap-1.5">
          {available.map((suggestion) => (
            <button key={`${suggestion.source ?? "env"}:${suggestion.key}`} type="button" title={suggestion.source ? `From ${suggestion.source}` : `Add ${suggestion.key}`} onClick={() => addSuggestion(suggestion)} className="rounded-md border border-border bg-background px-2 py-1 font-mono text-[11px] text-muted-foreground hover:bg-elevated hover:text-foreground">
              + {suggestion.key}{suggestion.source ? <span className="ml-1 font-sans text-subtle">· {suggestion.source}</span> : null}
            </button>
          ))}
        </div>
      ) : null}
      {inheritedRows.length ? (
        <div className="grid gap-2 rounded-md border border-border bg-elevated/40 p-2">
          <p className="text-[11px] text-subtle">Inherited from Cube · edit these in Cube configuration</p>
          {inheritedRows.filter((row) => row.key.trim()).map((row) => (
            <div key={`inherited-${row.key}`} className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_2.75rem]">
              <Input value={row.key} disabled aria-label="Inherited variable name" className="font-mono text-xs" />
              <Input value={row.value} disabled aria-label={`Inherited value for ${row.key}`} className="font-mono text-xs" />
              <span className="grid place-items-center text-[10px] text-subtle">Cube</span>
            </div>
          ))}
        </div>
      ) : null}
      <div className="hidden grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_2.75rem] gap-2 px-0.5 text-xs text-subtle sm:grid"><span>Name</span><span>Value</span><span className="sr-only">Remove</span></div>
...
export function parseMountLines(raw: string): MountRow[] {
  const rows = raw.split("\n").map((line) => line.trim()).filter(Boolean).map((line) => ({ id: rid(), ...parseMountLine(line) }));
  return rows.length ? rows : [{ id: rid(), source: "", destination: "", mode: "rw" }];
}

export function joinMountLines(rows: MountRow[]) {
  return rows.filter((r) => r.source.trim() && r.destination.trim()).map((r) => `${r.source}:${r.destination}:${r.mode}`).join("\n");
}
...
export function MountEditor({ rows, onChange }: { rows: MountRow[]; onChange: (rows: MountRow[]) => void; }) {
  ...
  return (
    <div className="grid gap-2">
      <div className="flex items-center justify-between gap-2"><Label>Mounts</Label><Button type="button" variant="ghost" size="sm" onClick={() => onChange([...rows, { id: rid(), source: "", destination: "", mode: "rw" }])}><Plus className="size-3.5" />Add</Button></div>
      <ul className="grid gap-2">
        {rows.map((row) => (
          <li key={row.id} className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)_4.5rem_2.75rem]">
            <Input value={row.source} onChange={(e) => patch(row.id, { source: e.target.value })} placeholder="D:\\wslc\\workspaces" aria-label="Host path" className="font-mono text-xs" spellCheck={false} />
            <Input value={row.destination} onChange={(e) => patch(row.id, { destination: e.target.value })} placeholder="/workspace" aria-label="Container path" className="font-mono text-xs" spellCheck={false} />
            <button type="button" className="h-11 rounded-md border border-border bg-background font-mono text-xs text-muted-foreground hover:bg-elevated hover:text-foreground" onClick={() => patch(row.id, { mode: row.mode === "rw" ? "ro" : "rw" })} aria-label={`Mode ${row.mode}`}>{row.mode}</button>
            <Button type="button" variant="ghost" size="icon" aria-label="Remove mount" onClick={() => { const next = rows.filter((r) => r.id !== row.id); onChange(next.length ? next : [{ id: rid(), source: "", destination: "", mode: "rw" }]); }}><Trash2 className="size-3.5" /></Button>
          </li>
        ))}
      </ul>
    </div>
