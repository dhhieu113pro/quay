  ...
  return <div className="grid gap-2">
    ...
    <ul className="grid max-h-56 gap-2 overflow-y-auto pr-1">{rows.map((row) => {
      ...
      return <li key={row.id} className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)_5.5rem_2.75rem]">
        ...
        <span className={required && !row.value.trim() ? "grid place-items-center rounded-md border border-destructive/40 px-1 text-center text-[10px] text-destructive" : "grid place-items-center text-center text-[10px] text-subtle"}>{source ?? (key ? "Custom" : "")}</span>
        {!protectedKeys.has(key) ? <Button type="button" variant="ghost" size="icon" aria-label="Remove variable" disabled={rows.length <= 1 && !row.key && !row.value} onClick={() => { const next = rows.filter((r) => r.id !== row.id); onChange(next.length ? next : [{ id: rid(), key: "", value: "" }]); }}><Trash2 className="size-3.5" /></Button> : <span aria-label="Protected container variable" />}
      </li>;
    })}</ul>
  </div>;
}
