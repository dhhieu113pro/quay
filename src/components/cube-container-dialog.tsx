...
export function CubeContainerDialog({ cube, open, initialSpec, onOpenChange, onSave }: { cube: ContainerGroup | null; open: boolean; initialSpec?: RunSpec; onOpenChange: (open: boolean) => void; onSave: (spec: RunSpec) => void; }) {
  ...
  useEffect(() => { if (open) { setNameTouched(Boolean(initialSpec)); applySpec({ ...(initialSpec ?? defaultSpec), groupId: cube?.id }); } }, [open, cube?.id, initialSpec]);
  if (!cube) return null;
  ...
  function applyImage(image: string) {
    const withImageDefaults = applyImageDefaults(spec, image, nameTouched || editing);
    ...
    applySpec({ ...next, groupId: cubeId, workspacePath: generated ? defaultCubeContainerWorkspacePath(cubePath, next.name || "container") : spec.workspacePath });
  }

  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex max-h-[90dvh] max-w-2xl flex-col gap-0 overflow-hidden p-0">
      <DialogHeader className="border-b border-border px-5 py-4"><DialogTitle>{initialSpec ? "Edit Container" : `Add Container to ${cube.name}`}</DialogTitle><DialogDescription>Cube variables are inherited; known images get required environment automatically.</DialogDescription></DialogHeader>
      <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => { event.preventDefault(); const name = spec.name.trim(); if (!name) { toast.error("Container name is required inside a Cube"); return; } if (missing.length) { toast.error(`Fill required environment: ${missing.join(", ")}`); return; } onSave({ ...spec, name, groupId: cube.id, env: localEnv, mounts: joinMountLines(mountRows), workspacePath, workspaceTarget: spec.workspaceTarget?.trim() || DEFAULT_WORKSPACE_TARGET }); onOpenChange(false); }}>
        <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 py-4">
          <div className="grid gap-1.5"><Label>Quick pick</Label><div className="flex flex-wrap gap-1.5">{catalogPresets.map((preset) => <button key={preset.image} type="button" onClick={() => { setNameTouched(false); const presetSpec = specFromPreset(preset); applySpec({ ...presetSpec, env: applyRuntimeEnvDefaults(presetSpec.env, preset.image), groupId: cube.id }); }} className={cn("h-9 rounded-md border px-3 text-xs", spec.image === preset.image ? "border-foreground bg-elevated text-foreground" : "border-border text-muted-foreground hover:bg-elevated/70")}>{preset.label}</button>)}</div>{selectedPreset ? <p className="text-xs text-subtle">{selectedPreset.hint}</p> : null}</div>
