    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90dvh] max-w-2xl flex-col gap-0 overflow-hidden p-0">
        <DialogHeader className="border-b border-border px-5 py-4">
          ...
          <DialogDescription>
            Cube environment variables are inherited by this container and can only be changed in Cube configuration.
          </DialogDescription>
        </DialogHeader>
        <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => {
          ...
          <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 py-4">
            <div className="grid gap-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {catalogPresets.map((preset) => (
                  <button key={preset.image} type="button" onClick={() => applySpec({ ...specFromPreset(preset), groupId: cube.id })} className={cn("h-9 rounded-md border px-3 text-xs", spec.image === preset.image ? "border-foreground bg-elevated text-foreground" : "border-border text-muted-foreground hover:bg-elevated/70")}>
                    {preset.label}
                  </button>
                ))}
              </div>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="grid gap-1.5"><Label htmlFor="cube-container-name">Name</Label><Input id="cube-container-name" value={spec.name} onChange={(event) => patch({ name: event.target.value })} placeholder="api" required /></div>
          ...
          <div className="grid gap-3 border-t border-border px-5 py-3">
            ...
            <p className="font-mono text-[11px] text-subtle">Cube network: {cube.network} · {inheritedRows.length} inherited env</p>
