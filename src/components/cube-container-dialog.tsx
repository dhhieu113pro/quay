...
export function CubeContainerDialog({ cube, open, initialSpec, onOpenChange, onSave }: { cube: ContainerGroup | null; open: boolean; initialSpec?: RunSpec; onOpenChange: (open: boolean) => void; onSave: (spec: RunSpec) => void; }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex max-h-[90dvh] max-w-2xl flex-col gap-0 overflow-hidden p-0">
      ...
      <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => { event.preventDefault(); const name = spec.name.trim(); if (!name) { toast.error("Container name is required inside a Cube"); return; } if (missing.length) { toast.error(`Fill required environment: ${missing.join(", ")}`); return; } if (invalidPorts) return; onSave({ ...spec, name, groupId: cube.id, env: localEnv, mounts: joinMountLines(mountRows), workspacePath, workspaceTarget: spec.workspaceTarget?.trim() || DEFAULT_WORKSPACE_TARGET }); onOpenChange(false); }}>
