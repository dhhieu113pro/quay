...
export function CubeContainerDialog({ cube, open, onOpenChange, onSave }: {
  cube: ContainerGroup | null; open: boolean; onOpenChange: (open: boolean) => void; onSave: (spec: RunSpec) => void;
}) {
  ...
  useEffect(() => { if (open) applySpec({ ...initialSpec, groupId: cube?.id }); }, [open, cube?.id]);
  if (!cube) return null;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="flex max-h-[90dvh] max-w-2xl flex-col gap-0 overflow-hidden p-0">
        ...
        <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => {
          ...
          onSave({
            ...spec, name, groupId: cube.id,
            env: withoutEnvKeys(joinEnvLines(envRows), cube.env), mounts: joinMountLines(mountRows),
          ...
          <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 py-4">
            <div className="grid gap-1.5">
              ...
              <div className="flex flex-wrap gap-1.5">
                {catalogPresets.map((preset) => (
                  <button key={preset.image} type="button" onClick={() => applySpec({ ...specFromPreset(preset), groupId: cube.id })} className={cn("h-9 rounded-md border px-3 text-xs", spec.image === preset.image ? "border-foreground bg-elevated text-foreground" : "border-border text-muted-foreground hover:bg-elevated/70")}>{preset.label}</button>
                ))}
              </div>
            ...
            <div className="grid gap-1.5">
              ...
              <Input id="cube-container-image" list="cube-image-catalog" value={spec.image} onChange={(event) => patch({ image: event.target.value })} required className="font-mono text-xs" />
              <datalist id="cube-image-catalog">{localImages.map((image) => <option key={image} value={image} />)}</datalist>
            </div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="grid gap-1.5">
                <Label htmlFor="cube-container-name">Name</Label>
                <Input id="cube-container-name" value={spec.name} onChange={(event) => {
            ...
            <div className="grid gap-2 rounded-lg border border-border p-3">
              ...
              <div className="grid gap-1.5 sm:max-w-xs">
                <Label htmlFor="cube-workspace-target">Container destination</Label>
                <Input id="cube-workspace-target" value={spec.workspaceTarget || DEFAULT_WORKSPACE_TARGET} onChange={(event) => patch({ workspaceTarget: event.target.value })} className="font-mono text-xs" />
              </div>
            </div>
          ...
          <div className="grid gap-3 border-t border-border px-5 py-3">
            ...
            <p className="font-mono text-[11px] text-subtle">Cube network: {cube.network} · {inheritedRows.length} inherited env · workspace → {spec.workspaceTarget || DEFAULT_WORKSPACE_TARGET}</p>
            <div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="submit">Add to Cube</Button></div>
          </div>
        </form>
      </DialogContent>
    </Dialog>
