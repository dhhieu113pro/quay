...
export function CubeContainerDialog({ cube, open, initialSpec, onOpenChange, onSave }: { cube: ContainerGroup | null; open: boolean; initialSpec?: RunSpec; onOpenChange: (open: boolean) => void; onSave: (spec: RunSpec) => void; }) {
  ...
  useEffect(() => { if (open) { inspectRequest.current += 1; setImageInspect(null); setNameTouched(Boolean(initialSpec)); applySpec({ ...(initialSpec ?? defaultSpec), groupId: cube?.id }); } }, [open, cube?.id, initialSpec]);
  if (!cube) return null;
