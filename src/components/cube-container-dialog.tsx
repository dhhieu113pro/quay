...
import { catalogPresets } from "@/lib/wslc/catalog";
...
const catalogSuggestions = catalogPresets.map((preset) => ({ reference: preset.image, label: preset.label, description: preset.hint }));
...
export function CubeContainerDialog({ cube, open, initialSpec, onOpenChange, onSave }: { cube: ContainerGroup | null; open: boolean; initialSpec?: RunSpec; onOpenChange: (open: boolean) => void; onSave: (spec: RunSpec) => void; }) {
  ...
  const [imageReady, setImageReady] = useState(Boolean(initialSpec?.image.trim()));
  ...
  useEffect(() => { if (open) { inspectRequest.current += 1; setImageInspect(null); setImageVolumeSources({}); setNameTouched(Boolean(initialSpec)); setImageDownloading(false); setImageReady(Boolean(initialSpec?.image.trim())); applySpec({ ...(initialSpec ?? defaultSpec), groupId: cube?.id }); } }, [open, cube?.id, initialSpec]);
  if (!cube) return null;
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex max-h-[90dvh] max-w-2xl flex-col gap-0 overflow-hidden p-0">
      <DialogHeader className="border-b border-border px-5 py-4"><DialogTitle>{initialSpec ? "Edit Container" : `Add Container to ${cube.name}`}</DialogTitle><DialogDescription>Search local images or download from Docker Hub and GHCR. Suggested images use the same safe download flow.</DialogDescription></DialogHeader>
      <form className="flex min-h-0 flex-1 flex-col" onSubmit={(event) => { event.preventDefault(); if (imageDownloading || !imageReady) return; const name = spec.name.trim(); if (!name) { toast.error("Container name is required inside a Cube"); return; } if (missing.length) { toast.error(`Fill required environment: ${missing.join(", ")}`); return; } if (invalidPorts) return; onSave({ ...spec, name, groupId: cube.id, env: localEnv, mounts: joinMountLines(mountRows), workspacePath, workspaceTarget: spec.workspaceTarget?.trim() || DEFAULT_WORKSPACE_TARGET }); onOpenChange(false); }}>
        <div className="grid min-h-0 flex-1 gap-4 overflow-y-auto px-5 py-4">
          <div className="grid gap-1.5"><Label htmlFor="cube-container-image">Image</Label><ImagePicker inputId="cube-container-image" value={spec.image} localImages={localImages} suggestedImages={catalogSuggestions} onSelect={applyImage} onBusyChange={setImageDownloading} onReadyChange={setImageReady} required /></div>
