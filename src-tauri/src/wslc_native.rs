...
type InitSessionSettings =
    unsafe extern "system" fn(*const u16, *const u16, *mut WslcSessionSettings) -> HResult;
...
type CreateSession =
    unsafe extern "system" fn(*mut WslcSessionSettings, *mut WslcSession, *mut *mut u16) -> HResult;
...
type PullSessionImage =
    unsafe extern "system" fn(WslcSession, *const WslcPullImageOptions, *mut *mut u16) -> HResult;
type InitProcessSettings = unsafe extern "system" fn(*mut WslcProcessSettings) -> HResult;
type SetProcessCmdLine =
    unsafe extern "system" fn(*mut WslcProcessSettings, *const *const c_char, usize) -> HResult;
type SetProcessEnv =
    unsafe extern "system" fn(*mut WslcProcessSettings, *const *const c_char, usize) -> HResult;
type SetProcessWorkingDirectory =
    unsafe extern "system" fn(*mut WslcProcessSettings, *const c_char) -> HResult;
type InitContainerSettings =
    unsafe extern "system" fn(*const c_char, *mut WslcContainerSettings) -> HResult;
type SetContainerName =
    unsafe extern "system" fn(*mut WslcContainerSettings, *const c_char) -> HResult;
type SetContainerInitProcess =
    unsafe extern "system" fn(*mut WslcContainerSettings, *mut WslcProcessSettings) -> HResult;
type SetContainerNetworkingMode =
    unsafe extern "system" fn(*mut WslcContainerSettings, i32) -> HResult;
type SetContainerPortMappings = unsafe extern "system" fn(
    *mut WslcContainerSettings,
    *const WslcContainerPortMapping,
    u32,
) -> HResult;
type SetContainerVolumes = unsafe extern "system" fn(
    *mut WslcContainerSettings,
    *const WslcContainerVolume,
    u32,
) -> HResult;
type CreateContainer = unsafe extern "system" fn(
    WslcSession,
    *const WslcContainerSettings,
    *mut WslcContainer,
    *mut *mut u16,
) -> HResult;
type SetProcessCallbacks = unsafe extern "system" fn(
...
unsafe fn check_with_error(hr: HResult, error: *mut u16, context: &str) -> Result<(), String> {
    if failed(hr) {
        ...
        Err(format!(
            "{context} failed: HRESULT 0x{:08X}: {detail}",
            hr as u32
        ))
    } else {
        if !error.is_null() {
            let _ = take_wide_error(error);
        }
        Ok(())
    }
}
...
unsafe extern "system" fn on_exit(code: i32, context: *mut c_void) {
    ...
    let ctx = &*(context as *const CallbackContext);
    if let Ok(mut value) = ctx.exit_code.lock() {
        *value = Some(code);
    }
}
...
pub struct NativeApi {
    ...
    set_process_callbacks: SetProcessCallbacks,
...
impl NativeApi {
    pub fn load() -> Result<Arc<Self>, String> {
        ...
        let api = unsafe {
            Self {
                ...
                set_container_init_process: symbol(
                    module,
                    b"WslcSetContainerSettingsInitProcess\0",
                )?,
                set_container_networking_mode: symbol(
                    module,
                    b"WslcSetContainerSettingsNetworkingMode\0",
                )?,
                set_container_port_mappings: symbol(
                    module,
                ...
                set_process_callbacks: symbol(module, b"WslcSetProcessSettingsCallbacks\0")?,
    ...
    pub fn create_session(
        ...
        std::fs::create_dir_all(storage)
            .map_err(|e| format!("create {}: {e}", storage.display()))?;
        let name_w = wide(OsStr::new(name));
        let storage_w = wide(storage.as_os_str());
        let mut settings = WslcSessionSettings { opaque: [0; 72] };
        unsafe {
            check(
                (self.init_session_settings)(name_w.as_ptr(), storage_w.as_ptr(), &mut settings),
                "WslcInitSessionSettings",
            )?;
            check(
                (self.set_session_cpu)(&mut settings, cpu),
                "WslcSetSessionSettingsCpuCount",
            )?;
            check(
                (self.set_session_memory)(&mut settings, memory_mb),
                "WslcSetSessionSettingsMemory",
            )?;
            let mut handle = null_mut();
            let mut error = null_mut();
            check_with_error(
                (self.create_session)(&mut settings, &mut handle, &mut error),
                error,
                "WslcCreateSession",
            )?;
            Ok(Session {
...
impl Session {
    pub fn pull(&self, image: &str) -> Result<(), String> {
        ...
        unsafe {
            let mut error = null_mut();
            check_with_error(
                (self.api.pull_session_image)(self.handle, &options, &mut error),
                error,
    ...
    pub fn create_container(&self, spec: &ContainerSpec) -> Result<Container, String> {
        ...
        let command = spec
            .command
            .iter()
            .map(|x| CString::new(x.as_str()).map_err(|e| e.to_string()))
        ...
        let callbacks = WslcProcessCallbacks {
            on_stdout: Some(on_output),
            on_stderr: Some(on_output),
            on_exit: Some(on_exit),
        };

        let volume_sources = spec
            .volumes
            .iter()
            .map(|v| wide(v.windows_path.as_os_str()))
            .collect::<Vec<_>>();
        let volume_destinations = spec
            .volumes
            .iter()
            .map(|v| CString::new(v.container_path.as_str()).map_err(|e| e.to_string()))
            .collect::<Result<Vec<_>, _>>()?;
        let native_volumes = spec
            .volumes
            .iter()
            .enumerate()
            .map(|(i, v)| WslcContainerVolume {
                windows_path: volume_sources[i].as_ptr(),
                container_path: volume_destinations[i].as_ptr(),
                read_only: if v.read_only { 1 } else { 0 },
            })
            .collect::<Vec<_>>();

        unsafe {
            check(
                (self.api.init_process_settings)(&mut process),
                "WslcInitProcessSettings",
            )?;
            check(
                (self.api.set_process_cmd_line)(&mut process, argv.as_ptr(), argv.len()),
                "WslcSetProcessSettingsCmdLine",
            )?;
            if !env_ptrs.is_empty() {
                check(
                    (self.api.set_process_env)(&mut process, env_ptrs.as_ptr(), env_ptrs.len()),
                    "WslcSetProcessSettingsEnvVariables",
                )?;
            }
            if !spec.workdir.is_empty() {
                check(
                    (self.api.set_process_workdir)(&mut process, workdir.as_ptr()),
                    "WslcSetProcessSettingsWorkingDirectory",
                )?;
            }
            check(
                (self.api.set_process_callbacks)(
                    &mut process,
                    &callbacks,
            ...
            check(
                (self.api.init_container_settings)(image.as_ptr(), &mut settings),
                "WslcInitContainerSettings",
            )?;
            check(
                (self.api.set_container_name)(&mut settings, name.as_ptr()),
                "WslcSetContainerSettingsName",
            )?;
            check(
                (self.api.set_container_init_process)(&mut settings, &mut process),
                "WslcSetContainerSettingsInitProcess",
            )?;
            check(
                (self.api.set_container_networking_mode)(
                    &mut settings,
            ...
            let ports = spec
                .ports
                .iter()
                .map(|&(host, container)| WslcContainerPortMapping {
                    windows_port: host,
            ...
            if !ports.is_empty() {
                check(
                    (self.api.set_container_port_mappings)(
                        &mut settings,
                        ports.as_ptr(),
                        ports.len() as u32,
                    ),
            ...
            if !native_volumes.is_empty() {
                check(
                    (self.api.set_container_volumes)(
                        &mut settings,
                        native_volumes.as_ptr(),
                        native_volumes.len() as u32,
                    ),
            ...
            let mut error = null_mut();
            check_with_error(
                (self.api.create_container)(self.handle, &settings, &mut handle, &mut error),
                error,
                "WslcCreateContainer",
            )?;
            Ok(Container {
...
impl Container {
    ...
    pub fn state(&self) -> Result<String, String> {
        unsafe {
            let mut state = 0;
            check(
                (self.api.get_container_state)(self.handle, &mut state),
                "WslcGetContainerState",
            )?;
            Ok(match state {
                WSLC_CONTAINER_STATE_CREATED => "created",
                WSLC_CONTAINER_STATE_RUNNING => "running",
                WSLC_CONTAINER_STATE_EXITED => "exited",
                WSLC_CONTAINER_STATE_DELETED => "deleted",
                _ => "invalid",
            }
            .into())
        }
    ...
    pub fn id(&self) -> Result<String, String> {
        ...
        unsafe {
            check(
                (self.api.get_container_id)(self.handle, buffer.as_mut_ptr()),
                "WslcGetContainerID",
            )?;
    ...
    pub fn inspect(&self) -> Result<String, String> {
        unsafe {
            ...
            check(
                (self.api.inspect_container)(self.handle, &mut ptr),
                "WslcInspectContainer",
            )?;
            Ok(take_ansi(ptr).unwrap_or_default())
        }
    }
    pub fn logs(&self) -> Vec<NativeLog> {
        self.logs.lock().map(|x| x.clone()).unwrap_or_default()
    }
    pub fn exit_code(&self) -> Option<i32> {
        self.exit_code.lock().ok().and_then(|x| *x)
    }
    pub fn stop(&self) -> Result<(), String> {
        ...
        unsafe {
            let mut error = null_mut();
            check_with_error(
                (self.api.stop_container)(self.handle, WSLC_SIGNAL_SIGTERM, 10, &mut error),
                error,
                "WslcStopContainer",
            )
        }
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            if !self.deleted {
                ...
                error = null_mut();
                let _ = (self.api.delete_container)(
                    self.handle,
                    WSLC_DELETE_CONTAINER_FLAG_FORCE,
                    &mut error,
                );
...
fn dll_candidates() -> Vec<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            ...
            result.push(dir.join("binaries").join("wslcsdk.dll"));
        }
    }
    if let Ok(current) = std::env::current_dir() {
        ...
        if current.ends_with("src-tauri") {
            if let Some(root) = current.parent() {
                result.push(root.join("src-tauri").join("binaries").join("wslcsdk.dll"));
            }
        }
    }
