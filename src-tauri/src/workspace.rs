...
fn normalize_absolute(path: &Path) -> Result<PathBuf, String> {
    if !path.is_absolute() { return Err("workspace path must be absolute".into()); }
...
fn validate_relative(path: &str) -> Result<PathBuf, String> {
    ...
    if path.trim().is_empty() || candidate.is_absolute() { return Err("workspace entry must be a relative path".into()); }
    let mut result = PathBuf::new();
    ...
    if result.as_os_str().is_empty() { return Err("workspace entry is empty".into()); }
    Ok(result)
}
...
fn validate_descendant(root: &Path, candidate: &Path) -> Result<(), String> {
    ...
    if candidate_key == root_key || candidate_key.starts_with(&format!(r"{}\", root_key)) { Ok(()) }
    else { Err("selected folder must be inside the Quay workspace root".into()) }
}
...
fn copy_dir_recursive(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    for entry in fs::read_dir(source).map_err(|e| format!("could not read {}: {e}", source.display()))? {
        ...
        if entry.file_type().map_err(|e| e.to_string())?.is_dir() { copy_dir_recursive(&source_path, &destination_path)?; }
        else { fs::copy(&source_path, &destination_path).map_err(|e| format!("could not copy {}: {e}", source_path.display()))?; }
    }
    Ok(())
}
...
fn move_without_overwrite(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    if let Some(parent) = destination.parent() { fs::create_dir_all(parent).map_err(|e| format!("could not create {}: {e}", parent.display()))?; }
...
fn default_workspace_root_from_app_data(app_data: &Path) -> PathBuf { app_data.join("workspace") }

#[tauri::command]
pub fn workspace_default_root(app: tauri::AppHandle) -> Result<String, String> {
    let app_data = app.path().app_data_dir().map_err(|e| format!("could not determine Quay app data directory: {e}"))?;
    Ok(default_workspace_root_from_app_data(&app_data).to_string_lossy().to_string())
}
...
fn pick_folder(current: Option<String>) -> Result<Option<String>, String> {
    ...
    let script = format!("Add-Type -AssemblyName System.Windows.Forms; $d=New-Object System.Windows.Forms.FolderBrowserDialog; $d.Description='Choose Quay workspace folder'; if ('{selected}' -ne '') {{$d.SelectedPath='{selected}'}}; if ($d.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {{[Console]::Write($d.SelectedPath)}}");
    let output = Command::new("powershell").args(["-NoProfile", "-STA", "-Command", &script]).stdin(Stdio::null()).output().map_err(|e| format!("could not open folder picker: {e}"))?;
...
pub fn workspace_open(root: String, relative: Option<String>) -> Result<(), String> {
    let root_path = normalize_absolute(Path::new(root.trim()))?;
    let target = if let Some(relative) = relative { let target = root_path.join(validate_relative(&relative)?); validate_descendant(&root_path, &target)?; target } else { root_path };
    fs::create_dir_all(&target).map_err(|e| format!("could not create {}: {e}", target.display()))?;
    #[cfg(windows)] Command::new("explorer.exe").arg(&target).spawn().map_err(|e| e.to_string())?;
    Ok(())
}
...
pub fn workspace_move_root(old_root: String, new_root: String) -> Result<(), String> {
    ...
    for managed in ["cubes", "containers"] {
        let source = old_root.join(managed); let destination = new_root.join(managed);
        if source.exists() && destination.exists() && fs::read_dir(&destination).map_err(|e| e.to_string())?.next().is_some() { return Err(format!("destination contains conflicting data: {}", destination.display())); }
    }
    for managed in ["cubes", "containers"] {
        let source = old_root.join(managed); let destination = new_root.join(managed);
        if source.exists() {
            if destination.exists() { fs::remove_dir(&destination).map_err(|e| format!("could not prepare {}: {e}", destination.display()))?; }
            move_without_overwrite(&source, &destination)?;
        } else { fs::create_dir_all(&destination).map_err(|e| e.to_string())?; }
    }
    Ok(())
}
...
mod tests {
    ...
    fn relative_validation_rejects_parent_segments() {
        assert!(validate_relative(r"cubes\demo").is_ok()); assert!(validate_relative(r"..\other").is_err()); assert!(validate_relative(r"C:\temp").is_err());
    }
    #[test]
    fn app_data_default_stays_under_quay_app_data() {
        let app_data = Path::new(r"C:\Users\me\AppData\Roaming\com.quay.app");
        assert_eq!(default_workspace_root_from_app_data(app_data), app_data.join("workspace"));
        assert_ne!(default_workspace_root_from_app_data(app_data), PathBuf::from(r"C:\Users\me\Quay"));
    }
    #[test]
    fn move_refuses_to_overwrite_destination() {
        let base = std::env::temp_dir().join(format!("quay-workspace-test-{}", std::process::id()));
        let source = base.join("source"); let destination = base.join("destination"); let _ = fs::remove_dir_all(&base);
        fs::create_dir_all(&source).unwrap(); fs::create_dir_all(&destination).unwrap(); assert!(move_without_overwrite(&source, &destination).is_err()); let _ = fs::remove_dir_all(&base);
    }
}
